...
const blake2b_ts_1 = require("./blake2b.js");
...
function Hp(A, dkLen) {
    ...
    // Fast path
    if (dkLen <= 64)
        return blake2b_ts_1.blake2b.create({ dkLen }).update(T8).update(A8).digest();
    const out = new Uint8Array(dkLen);
    let V = blake2b_ts_1.blake2b.create({}).update(T8).update(A8).digest();
    let pos = 0;
    // First block
    ...
    for (; dkLen - pos > 64; pos += 32) {
        const Vh = blake2b_ts_1.blake2b.create({}).update(V);
        Vh.digestInto(V);
        Vh.destroy();
        out.set(V.subarray(0, 32), pos);
    }
    // Last block
    out.set((0, blake2b_ts_1.blake2b)(V, { dkLen: dkLen - pos }), pos);
    V.fill(0);
    T.fill(0);
...
function argon2Init(password, salt, type, opts) {
    password = (0, utils_ts_1.toBytes)(password);
    ...
    //       LE32(length(X)) || X)
    const h = blake2b_ts_1.blake2b.create({});
