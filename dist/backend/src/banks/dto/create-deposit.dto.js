"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CreateDepositDto = void 0;
const class_validator_1 = require("class-validator");
const swagger_1 = require("@nestjs/swagger");
class CreateDepositDto {
    bankId;
    minAmount;
    interestRate;
    periodMonths;
    currency;
}
exports.CreateDepositDto = CreateDepositDto;
__decorate([
    (0, swagger_1.ApiProperty)({
        example: '507f1f77bcf86cd799439011',
        description: 'ID банка'
    }),
    (0, class_validator_1.IsNotEmpty)(),
    __metadata("design:type", String)
], CreateDepositDto.prototype, "bankId", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 10000, description: 'Минимальная сумма' }),
    (0, class_validator_1.IsNumber)(),
    __metadata("design:type", Number)
], CreateDepositDto.prototype, "minAmount", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 5.5, description: 'Процентная ставка' }),
    (0, class_validator_1.IsNumber)(),
    __metadata("design:type", Number)
], CreateDepositDto.prototype, "interestRate", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 12, description: 'Срок в месяцах' }),
    (0, class_validator_1.IsNumber)(),
    __metadata("design:type", Number)
], CreateDepositDto.prototype, "periodMonths", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({
        example: 'RUB',
        description: 'Валюта',
        enum: ['RUB', 'USD', 'EUR']
    }),
    (0, class_validator_1.IsIn)(['RUB', 'USD', 'EUR']),
    __metadata("design:type", String)
], CreateDepositDto.prototype, "currency", void 0);
//# sourceMappingURL=create-deposit.dto.js.map