...
const LoginPage = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const resultAction = await dispatch((0, authSlice_1.login)({ email, password }));
        if (authSlice_1.login.fulfilled.match(resultAction)) {
      ...
      <material_1.Box sx={{
        ...
        <material_1.Paper elevation={3} sx={{ p: 4, width: '100%' }}>
          ...
          <material_1.Box component="form" onSubmit={handleSubmit} noValidate>
            ...
            <material_1.TextField margin="normal" required fullWidth name="password" label="Пароль" type="password" id="password" autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} disabled={status === 'loading'}/>
            <material_1.Button type="submit" fullWidth variant="contained" sx={{ mt: 3, mb: 2 }} disabled={status === 'loading'}>
              {status === 'loading' ? <material_1.CircularProgress size={24}/> : 'Войти'}
            </material_1.Button>
            <material_1.Box sx={{ textAlign: 'center' }}>
              <react_router_dom_1.Link to="/register" style={{ textDecoration: 'none' }}>
                <material_1.Button color="primary">Нет аккаунта? Зарегистрируйтесь</material_1.Button>
